...
void ads1299_hw_init(void)
{
    ...
    if (ret != 0)
    {
        APP_LOG_INFO("SPI master dma init failed! Please check the input paraments.\r\n");
    }
...
int eeg_afe_hw_spi_dma_receive(uint8_t *p_tx_data, uint8_t *p_rx_data, uint32_t size)
{
	app_drv_err_t ret = 0;
